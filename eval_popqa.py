...
from setup_popqa_data import ensure_popqa, ensure_kilt_corpus
...
class PopQAArgs(LMArgs, RetrievalArgs):
    output_dir: str = field(default="data/results/popqa_hf_kilt")

    eval_data: str = field(
        default_factory=ensure_popqa,        # auto‑build PopQA jsonl
        metadata={'help': 'Path to PopQA test jsonl.'}
    )
    ...
    corpus: str = field(
        default_factory=ensure_kilt_corpus,  # auto‑build KILT corpus
        metadata={'help': 'Path to KILT jsonl corpus.'}
    )
